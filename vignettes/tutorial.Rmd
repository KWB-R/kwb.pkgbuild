...
default style. We would like to apply this default style to all KWB-packages
that are hosted on our GitHub account KWB-R. The aim is to present all packages
in a common manner (consistent DESCRIPTION and documentation) and to use the
same services (continuous integration, code coverage) for all our packages. 
...
will be set to the open source licence MIT. An additional LICENCE file will be 
added that informs about KWB being the copyright holder of the package.

# 1 Install required packages

Before you can use the package kwb.pkgdown you need to 

* install packages that kwb.pkgdown relies on (if not yet installed), from
[The Comprehensive R Archive Network (CRAN)](https://cloud.r-project.org),
* install kwb.pkddown from [GitHub](https://github.com/kwb-r/kwb.pkgdown).
...
The remotes package that should now be installed, allows to install packages 
directly from GitHub. Therefore, the functions from this package need to 
communicate with the GitHub server. As it is best for users to get explicitly authenticated by the GitHub server we will set a so called Private Access Token
(PAT) in the following. This token will then automatically be used whenever the
GitHub server is accessed. Please see the [Installation vignette](install.html)
first to learn how to get your personal GitHub PAT and use that token instead of
the <placeholder> in the following command:

```{r, eval = FALSE}
Sys.setenv(GITHUB_PAT = "<your personal GitHub PAT>")
```

Now, that the access token is set, you can use `install_github()` from the
remotes packages to install packages directly from GitHub. Let's start by 
installing a fully working version of the remotes package itself (unfortunately,
the current CRAN version as installed above contains a bug...): 

```{r, eval = FALSE}
remotes::install_github("r-lib/remotes@18c7302637053faf21c5b025e1e9243962db1bdc")
```

Finally we can install the latest version of this package from our GitHub 
account:
