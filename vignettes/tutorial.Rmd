...
Secondly you need to personalise your R package, but using a default KWB package 
build template that is wrapped into the R package `kwb.pkgbuild`.
...
However, it is also possible to additionally specify a personal website and an 
[ORCID](https://orcid.org/), for tracking your work as shown below:
...
- **title**: title of your R package in TitleCase (you can use the website 
https://titlecase.com/ for converting your non- title case conform title to a 
valid one)

- **desc:** is the package description. Should be at least one sentence long and 
needs to end with a point!
...
- Creates READMEs (.md, .Rmd) for the above mentioned topics and
...
button. A successful installation should finish with `Done` as shown below:
